...
export type LayoutTemplateId = BuiltInLayoutTemplateId | string;
...
export interface LayoutPresetDefinition {
  ...
  elementPrototypes: TemplateElementPrototypeData[];
}

export const COMMON_TEXT_PROTOTYPES: TemplateElementPrototypeData[] = [
  {
    slot: 'subtitle',
    defaultType: 'TEXT',
    zIndex: 1,
...
function buildPreset(
  ...
  const elementPrototypes = [...COMMON_TEXT_PROTOTYPES, ...imagePrototypes];
  const slots: LayoutSlotData[] = elementPrototypes.map((p) => ({
    name: p.slot,
    defaultTransform: p.transform,
    allowedTypes: [p.defaultType],
  }));

  return { id, name, description, slots, elementPrototypes };
}
