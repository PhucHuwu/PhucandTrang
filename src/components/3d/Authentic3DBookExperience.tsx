...
import { RealisticSkinnedBook } from './RealisticSkinnedBook';
import { AtmosphericSystem } from './AtmosphericSystem';
import { ChevronLeft, ChevronRight, BookOpen } from 'lucide-react';
import VintageMusicPlayer from '@/components/VintageMusicPlayer';

export default function Authentic3DBookExperience() {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const [currentPage, setCurrentPage] = useState(0); // 0 = closed cover
  const [totalPages, setTotalPages] = useState(0);
  const [isLoaded, setIsLoaded] = useState(false);
  const bookRef = useRef<RealisticSkinnedBook | null>(null);
  ...
  useEffect(() => {
    ...
    // 2. Warm Studio Lights
    const ambientLight = new THREE.AmbientLight(0xFFF9EE, 1.2);
    scene.add(ambientLight);

    const keyLight = new THREE.DirectionalLight(0xFFE6D0, 1.6);
    keyLight.position.set(3, 6, 5);
    keyLight.castShadow = true;
    scene.add(keyLight);

    const fillLight = new THREE.DirectionalLight(0xE8BCC6, 0.6);
    fillLight.position.set(-4, 3, 3);
    scene.add(fillLight);

    // 3. Atmospheric System (Butterflies & Dust)
    const atmospheric = new AtmosphericSystem(scene);

    // 4. Build Skinned Book with dynamic canvas textures
    const book = new RealisticSkinnedBook(scene);
    bookRef.current = book;

    const buildPages = async () => {
      // Front & Back Cover Textures
      const coverFront = PageTextureGenerator.createCoverTexture('CHÚNG MÌNH', 'Phúc & Trang', '20.10.2022');
      ...
      // Page 2: Chapter 2 (Confession 20.10.2022) & Photo 2
      ...
      // Page 3: Chapter 3 (Scrapbook Memories)
      ...
      // Page 4: Chapter 4 (Eternal & Letter)
      ...
      book.initPages(defs);
      setTotalPages(defs.length);
      setIsLoaded(true);
    };

    buildPages();

    // 5. Parallax Mouse Listener
    const handleMouseMove = (e: MouseEvent) => {
      mouseParallax.current.targetX = (e.clientX / window.innerWidth) * 2 - 1;
      mouseParallax.current.targetY = -(e.clientY / window.innerHeight) * 2 + 1;
    };
    ...
    const render = () => {
      ...
      // Soft book tilt (±2° X, ±4° Y)
      book.group.rotation.x = py * 0.035;
      book.group.rotation.y = px * 0.06;

      // Center offset adjustment: when open, shift rightwards slightly to center spread
      const targetGroupX = currentPage > 0 ? 0.3 : 0;
      book.group.position.x += (targetGroupX - book.group.position.x) * 0.08;

      book.update(dt);
      atmospheric.update(elapsed, { x: px, y: py });
  ...
  // Sync state to 3D book model
  const handleTurnPage = (index: number) => {
    if (!bookRef.current || index < 0 || index > totalPages) return;
    setCurrentPage(index);
    bookRef.current.setPageTurnProgress(index);
  };
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (e.deltaY > 30) {
        lastWheel = now;
        handleTurnPage(Math.min(totalPages, currentPage + 1));
      } else if (e.deltaY < -30) {
        lastWheel = now;
        handleTurnPage(Math.max(0, currentPage - 1));
      }
    };
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden select-none bg-[#110D0E]">
      ...
      <div
        ref={containerRef}
        onClick={() => {
          if (currentPage === 0) handleTurnPage(1);
        }}
        className="absolute inset-0 z-0 cursor-pointer"
        style={{
          background: 'radial-gradient(ellipse at center, #26171B 0%, #150D10 55%, #0A0607 100%)',
        }}
      />
      ...
      {isLoaded && (
        <div className="absolute inset-0 z-10 pointer-events-none flex flex-col justify-between p-4 sm:p-8">
          ...
          {/* Bottom Flip Buttons & Hints */}
          <div className="flex items-center justify-between max-w-5xl w-full mx-auto pointer-events-auto">
            <button
              onClick={() => handleTurnPage(Math.max(0, currentPage - 1))}
            ...
            <button
              onClick={() => handleTurnPage(Math.min(totalPages, currentPage + 1))}
